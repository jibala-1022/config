...
run() {
    ...
    case "$extension" in
    c)
	    [ ! -d "$exe_directory" ] && mkdir .exes
        gcc "$filename" -o "$exe_directory"/"$executable" && ./"$exe_directory"/"$executable" "${@:2}"
        ;;
    cpp)
	    [ ! -d "$exe_directory" ] && mkdir .exes
        g++ "$filename" -o "$exe_directory"/"$executable" && ./"$exe_directory"/"$executable" "${@:2}"
        ;;
    py)
        python3 "$filename" "${@:2}"
        ;;
